<template>
  <div class="tool-page">
    ...
    <div class="tool-body">
      <p class="label">选择图片，合并为一个 PDF 文件</p>
      <input type="file" accept="image/*" multiple @change="onFiles" ref="fileInput" />
      ...
      <div v-if="result" class="result">
        <p>✅ 转换完成</p>
        <button @click="download" class="btn secondary">下载 PDF</button>
      </div>
    </div>
  </div>
</template>
...
const { files, loading, error, result, fileInput, onFiles, removeFile, formatSize, execute, download } = usePdfTool({ tool: 'imagepdf', outputExt: 'pdf' })
</script>
